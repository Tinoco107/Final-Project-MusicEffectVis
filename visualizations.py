...
data["Correct"] = np.where(
    pd.isna(data["Response"]), False, data["Response"] == data["Correct_Response"]
)
...
trial_trend = (
    data.groupby(["Session", "TrialNumber"])["Response_Time"].mean().reset_index()
)

plt.figure(figsize=(12, 6))
sns.lineplot(
    data=trial_trend, x="TrialNumber", y="Response_Time", hue="Session", marker="o"
)
...
data["Is_Target"] = (
    data.groupby(["Subject", "Session"])["Stimulus_Letter"].shift(1)
    == data["Stimulus_Letter"]
)
